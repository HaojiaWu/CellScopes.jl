...
```
## 2. Tutorials
On this markdown page, we will demonstrate how to use ```CellScopes.jl``` to analyze a Cartana dataset generated by 10x Genomics (Similar to Xenium). Additionally, CellScopes also supports analysis for single-cell RNA sequencing (scRNA-seq) and Visium datasets. For more information, please refer to the tutorials provided below.
<br>
...
### 3.3. How to construct a SpaObj object?
...
```julia
kidney = cs.normalizeData(kidney);
```
