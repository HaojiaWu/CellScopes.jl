...
```
### 3. Generate cell polygons and assign gene expression to the polygons
We used the ```boundary_polygons``` function in Baysor to draw the cell polygons. To assign gene expression values to the cell polygons, we associated each polygon with the cell closest to it based on Euclidean distance.
