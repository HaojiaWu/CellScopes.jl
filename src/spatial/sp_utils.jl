...
function compute_pearson_cor(sp::get_object_group("Imaging"), cluster1::Union{Symbol, String}, cluster2::Union{Symbol, String}; color_scheme::String="lightgreyred",reverse_color::Bool=false)
...
function compare_cell_distances(sp::get_object_group("Imaging"), col::Union{String, Symbol}, target_cell::String, 
...
function compute_kidney_coordinates(sp::get_object_group("Spatial"), center)
    df = sp.spmetaData.cell
    kid_depth=[]
    kid_angle=[]
    n=length(df.x)
...
function bin_gene_spatial(sp::get_object_group("Imaging"), n_bin::Int64; 
    ...
    version::String = "v5")
        ...
        elseif data_type == "spatial"
            ...
            elseif tech == "xenium"
                ...
                counts = sparse_r_to_jl(counts)
                cells = rcopy(rbase.colnames(seu_obj["assays"][assay]))
                genes = rcopy(rbase.rownames(seu_obj["assays"][assay]))
                raw_count = RawCountObject(counts, cells, genes)
                x_coords = Float64[]
                y_coords = Float64[]
                gene_symbols = String[]
                ...
                cs_obj = XeniumObject(molecules, cell_coords, raw_count; meta_data=meta)
                cs_obj.polygonData = poly
                cs_obj = normalize_object(cs_obj)
                elseif tech == "visium"
                if version=="v5"
                    assay = "Spatial"
                ...
                values_scale = [1 , 1]
                if version =="v5"
                    scale_hi = rcopy(seu_obj["images"][1]["scale.factors"]["hires"])
                    scale_low = rcopy(seu_obj["images"][1]["scale.factors"]["lowres"])
                    values_scale = [scale_low , scale_hi]
                end
        ...
        nothing, 
        nothing, nothing, nothing )
        if version == "v5" && tech == "visium"
        cs_obj = subset_object(cs_obj; cells = cells_kept)
    end
    return cs_obj
end
